// src/sampleFromTokens.ts
import { Token } from "./types";

// Sample data for tokens from various chains (all tokens use the same format)
const sampleFromTokensData: { [chainId: string]: { [address: string]: Token } } = {
eth: {
...
export const sampleFromTokens: { [chainId: string]: Token[] } = {};
for (const chainId in sampleFromTokensData) {
	sampleFromTokens[chainId] = Object.keys(sampleFromTokensData[chainId]).map(
		(address) => sampleFromTokensData[chainId][address]
	);
}
