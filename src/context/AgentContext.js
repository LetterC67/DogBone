...
async function getTokenByAddress(address, chainId) {
    const token = tokenList.tokens.find((token) => token.address === address && token.chainId === chainId);
    if (token) {
        return token;
    }
    return null;
}
async function getTokenBySymbol(symbol, chainId) {
...
export const AgentProvider = ({ children }) => {
    ...
    const { setFromTokenSwap, setToTokenSwap, setFromAmountSwap, setFromAmountBridge, setFromTokenBridge, setToTokenBridge, setFromChainBridge, setIsInStrategyTab, setStrategyAmount, setStrategyToken, setStrategy, setFilteredStrategies, setShowOnlyDeposited, setAgentFilteredStrategies, setStrategyChain, setStrategyTab } = useControl();
    ...
    const { strategyList, threadID } = useData();
    ...
    function containsFunction(code) {
    ...
    async function __SET_STRATEGIES__(strategies) {
    ...
    async function __GET_BALANCE__(token, chainId) {
    ...
    async function __MESSAGE__(message) {
        console.log(message);
        if (isAnswering)
            return;
    ...
    return (_jsx(AgentContext.Provider, { value: {
            messages,
            userMessages,
            agentMessages,
            sendMessage,
