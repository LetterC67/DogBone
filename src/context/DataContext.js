...
import { getVaultPosition } from "../tools/ToolAPI";
...
import { getTokenPriceByAddress } from "../tools/utils/getTokenPrice";
...
// Create a context
const DataContext = createContext({ tokenLists: [], loading: true, aprList: {}, strategyList: [], tokenPriceSonic: {}, getTokenListByChainId: (chainId) => { }, sonicPoint: 0, ringsPoint: 0, threadID: "", depositedSonic: {}, tokenBalanceSonic: {}, refetchBalance: (token) => { }, refetchPosition: (strategy) => { } });
export const DataProvider = ({ children }) => {
    const [loading, setLoading] = useState(true);
    const [tokenLists, setTokenLists] = useState({});
    const [aprList, setAprList] = useState({});
    const [strategyList, setStrategyList] = useState([]);
    const [tokenPriceSonic, setTokenPriceSonic] = useState({});
    const [tokenBalanceSonic, setTokenBalanceSonic] = useState({});
    const [depositedSonic, setDepositedSonic] = useState({});
    ...
    function getTokenListByChainId(chainId) {
        return tokenList.tokens.filter((token) => token.chainId === chainId);
    }
    function changeForm(data) {
        const result = {};
        for (const chainId in data) {
            result[chainId] = Object.keys(data[chainId]).map((address) => data[chainId][address]);
        }
        return result;
    }
    useEffect(() => {
    ...
    useEffect(() => {
        async function fetchAllPrices() {
            ...
            // console.log(prices);
            // convert key tolower
            let prices_ = Object.fromEntries(Object.entries(prices).map(([key, value]) => [key.toLowerCase(), value]));
            ...
            const result = Object.fromEntries(Object.entries(prices_).map(([address, price]) => {
    ...
    return (_jsx(DataContext.Provider, { value: { tokenLists, loading, aprList, strategyList, tokenPriceSonic, getTokenListByChainId, tokenBalanceSonic, sonicPoint, ringsPoint, threadID, depositedSonic, refetchBalance, refetchPosition }, children: children }));
};
