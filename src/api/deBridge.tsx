...
async function getTokenList(chainId: string | number) {
    const endpoint = `${URL}/v1.0/token-list?chainId=${chainId}`;

    return await fetch(endpoint).then((res) => res.json()).then((data) => data.tokens);
}

async function createTx(srcChainId: string | number, srcChainTokenIn: string, srcChainTokenInAmount: bigint, dstChainId: number | string, dstChainTokenOut: string){
