import { createPublicClient, custom, encodeFunctionData, formatUnits, parseUnits, } from 'viem';
...
export async function depositRingsSC({ walletClient, vaultAddress, amount, }) {
    ...
    const parsedAmount = parseUnits(amount, await getERC20Decimals({ publicClient, tokenAddress: scToken }));
...
export async function depositRingsNative({ walletClient, vaultAddress, amount, }) {
    if (walletClient.chainId.slice(7, walletClient.chainId.length) !==
        sonic.id.toString()) {
        await walletClient.switchChain(sonic.id);
    }
