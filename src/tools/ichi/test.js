...
export function dak() {
    const list = [];
    const ap = [];
    ichiVaultList.map((vault) => {
        const token = tokenList.tokens.find((token) => token.address.toLowerCase() === vault.token.toLowerCase());
        if (!token) {
            if (!list.includes(vault.token))
                list.push(vault.token);
...
export async function dakmim() {
    ...
    for (const vault of ichiVaultList) {
        const tokenIn = tokenList.tokens.find((token) => token.address.toLowerCase() === vault.token.toLowerCase());
        let tokenDak;
        ...
        const tokenDakConfig = tokenList.tokens.find((token) => token.address.toLowerCase() === tokenDak.toLowerCase());
        if (tokenDakConfig) {
            list.push({
