...
export async function viewLTokenPosition({ vaultAddress, userAddress, }) {
    ...
    const userShareBalance = results[0].result;
    const vaultSupply = results[1].result;
    const debondFee = results[2].result;
    const vaultBalance = (await getERC20Balance({
        publicClient,
        account: vaultAddress,
        tokenAddress: lConfig.token,
    }));
    const perc = (((userShareBalance * (DEN - debondFee)) / DEN) * FIXED_POINT_Q96) /
        vaultSupply;
    const userUnderlyingBalance = (vaultBalance * perc) / FIXED_POINT_Q96;
    const formattedUserUnderlyingBalance = formatUnits(userUnderlyingBalance, await getERC20Decimals({
        publicClient,
        tokenAddress: lConfig.token,
    }));
