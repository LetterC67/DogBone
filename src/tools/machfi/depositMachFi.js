...
export async function depositMachFi({ walletClient, vaultAddress, amount, }) {
    ...
    const parsedAmount = parseUnits(amount, await getERC20Decimals({ publicClient, tokenAddress: token }));
    ...
    const transactionRequest = token === NATIVE_TOKEN
...
function getNativeTransactionRequest(vaultAddress, amount) {
...
function getERC20TransactionRequest(vaultAddress, amount) {
    const transactionData = encodeFunctionData({
        abi: machFiERC20Abi,
        functionName: 'mintAsCollateral',
        args: [amount],
